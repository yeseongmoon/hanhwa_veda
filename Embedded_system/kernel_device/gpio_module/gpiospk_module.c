...
/* 디바이스 파일의 주 번호와 부 번호 */
#define GPIO_MAJOR 204
#define GPIO_MINOR 0
#define GPIO_DEVICE "gpiospk" /* 디바이스 디바이스 파일의 이름 */

#define GPIO_SPK 539 /* LED 사용을 위한 GPIO의 번호 */
...
int init_module(void) {
  ...
  /* GPIO 사용을 요청한다. */
  gpio_request(GPIO_SPK, "SPEAKER");
  gpio_direction_output(GPIO_SPK, 0);
...
static void playTone(int frequency, int duration) {
  int half_period = 500000 / frequency; // 반주기(마이크로초)
  int cycles = (duration * 1000) / (2 * half_period);
  for (int i = 0; i < cycles; i++) {
    gpio_set_value(GPIO_SPK, 1);
    udelay(half_period);
