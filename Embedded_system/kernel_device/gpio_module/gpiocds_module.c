...
/* 디바이스 파일의 주 번호와 부 번호 */
#define GPIO_MAJOR 202
#define GPIO_MINOR 0
#define GPIO_DEVICE "gpiocds" /* 디바이스 디바이스 파일의 이름 */

#define GPIO_CDS 529 /* LED 사용을 위한 GPIO의 번호 */
...
int init_module(void) {
  ...
  /* GPIO 사용을 요청한다. */
  gpio_request(GPIO_CDS, "CDS");
  gpio_direction_input(GPIO_CDS);
...
static ssize_t gpio_read(struct file *inode, char *buff, size_t len,
  ...
  if (gpio_get_value(GPIO_CDS) == 0) {
    strcat(msg, "0");
  }

  else if (gpio_get_value(GPIO_CDS) == 1) {
    strcat(msg, "1");
  }
